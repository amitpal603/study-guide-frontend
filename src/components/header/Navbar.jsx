...
function Navbar() {
  ...
  const { role, logoutHandler } = useContext(userAuth); // make sure logout exists in context
  ...
  const handleAccountChange = (e) => {
    ...
    if (value === "SignUp") navigate("/sign-up");
    else if (value === "Login") navigate("/login-account");
    else if (value === "Logout") {
      logoutHandler(); // call logout from context
      navigate("/");
    }
  ...
  const handleUniversityChange = (e) => {
    const value = e.target.value;
    if (value === "Mgkvp") navigate("/mgkvp-university");
    else if (value === "Aktu") navigate("/aktu-university");
  ...
  const dropdownClass =
    "bg-white text-[#F67280] font-medium text-sm lg:text-base px-3 lg:px-5 py-2 lg:py-2.5 rounded-md border-2 border-white shadow-md hover:bg-gray-50 hover:shadow-lg transition-all duration-200 cursor-pointer focus:outline-none focus:ring-2 focus:ring-white focus:ring-opacity-60 appearance-none pr-8 lg:pr-10 bg-[length:1.2em] lg:bg-[length:1.5em] bg-[right_0.4rem_center] lg:bg-[right_0.5rem_center] bg-no-repeat";
          ...
          <div className="flex items-center justify-between h-20 gap-2">
            ...
            <div className="flex-shrink-0 flex gap-2 sm:gap-3 justify-center items-center">
              <div className="h-12 w-12 sm:h-16 sm:w-16 rounded-full overflow-hidden border-2 border-white shadow-md hover:scale-105 transition-transform duration-200">
                <img src={study} alt="Study" className="h-full w-full object-cover" />
              </div>
            ...
            <ul className="hidden md:flex space-x-2 lg:space-x-4 flex-1 justify-center md:ml-85">
              ...
              {isAdmin && (
                <li>
                  <NavLink to="/admin/study-guide/dashboard" className={navLinkClass}>
                    Admin Dashboard
                  </NavLink>
                </li>
              )}
            ...
            <div className="hidden md:block flex-shrink-0">
              <select onChange={handleUniversityChange} className={dropdownClass} style={dropdownStyle}>
            ...
            {/* Account Dropdown - role-based options */}
            <div className="hidden md:block flex-shrink-0">
              <select onChange={handleAccountChange} className={dropdownClass} style={dropdownStyle}>
        ...
        <nav className="p-4">
          <ul className="space-y-2">
            {links.map((link, index) => (
              ...
              >
                <NavLink to={link.to} onClick={() => setIsOpen(false)} className={mobileNavLinkClass}>
                  {link.label}
                </NavLink>
              </li>
            ))}
            {isAdmin && (
              <li onClick={() => setIsOpen(false)}>
                <NavLink to="/admin/study-guide/dashboard" className={mobileNavLinkClass}>
                  Admin Dashboard
                </NavLink>
              </li>
            )}
          ...
          <div className="mt-6 pt-4 border-t border-white border-opacity-30">
            <h3 className="text-white text-sm font-semibold mb-3 px-2 uppercase tracking-wide">University</h3>
            <select
          ...
          {/* Account - Mobile */}
          <div className="mt-6 pt-4 border-t border-white border-opacity-30">
            <h3 className="text-white text-sm font-semibold mb-2 px-2 uppercase tracking-wide">Account</h3>
              ...
              {!isUser && !isAdmin && (
                <>
                  ...
                  <NavLink
                    to="/login-account"
                    onClick={() => setIsOpen(false)}
                    className="block w-full text-left text-white font-semibold text-base py-3 px-4 rounded-lg hover:bg-[#e55d6d] hover:bg-opacity-70 transition-all duration-200 active:scale-95"
                  >
                    Login
                  </NavLink>
                </>
